...
n_vars = (num_sensors+1) * n_out  # (num_sensors+1)*n_hidden_neurons + (n_hidden_neurons+1)*n_out
...
if hardware:
    ...
    # rob = robobo.SimulationRobobo().connect(address='172.20.10.6', port=port)  # 19997
...
def eval(x):
    ...
    # prey_robot = robobo.SimulationRoboboPrey().connect(address='172.20.10.6', port=19989)
    prey_controller = prey.Prey(robot=prey_robot, level=2)
    prey_controller.start()
    ...
    while sim_length_ms > elapsed_time:
        ...
        food_old = np.vstack([food, food_old[:((num_sensors/3) - 1)]])
        print("food buffer:")
        print(food_old)
